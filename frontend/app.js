...
async function renderDashboard() {
  ...
  const categoryBars = byCategory
    .map(
      ([name, count]) => `
      <button class="bar-item clickable" data-filter-type="category" data-filter-value="${name}">
        <div class="meta"><span>${name}</span><span>${count}</span></div>
  ...
  el.pageDashboard.innerHTML = `
    ...
    <div class="metrics">
      ...
      <div class="metric"><div class="label">Q1任务</div><div class="value">${stats.by_quadrant[1]}</div></div>
      <div class="metric"><div class="label">Q2任务</div><div class="value">${stats.by_quadrant[2]}</div></div>
...
async function renderQuadrant() {
  ...
  const selectedCategory = state.categories.includes(state.taskFilters.category)
  ...
  const selectedQuadrant = ["全部", "1", "2", "3", "4"].includes(state.taskFilters.quadrant)
    ...
    : "全部";
  const selectedSort = ["created_desc", "created_asc", "updated_desc", "updated_asc"].includes(
  ...
  page.innerHTML = `
    <h1>四象限任务</h1>
    <div class="filters">
      <input id="flt-keyword" placeholder="搜索标题或描述" value="${state.taskFilters.keyword}" />
      <select id="flt-category"><option ${selectedCategory === "全部" ? "selected" : ""}>全部</option>${state.categories
        .map((c) => `<option ${selectedCategory === c ? "selected" : ""}>${c}</option>`)
        .join("")}</select>
  ...
  async function refresh() {
    ...
    state.taskFilters = { keyword, category, quadrant, sort };
    const q = new URLSearchParams({ keyword, category, quadrant, sort });
    ...
    data.items.forEach((task) => {
      ...
      dot.className = `task-dot q${task.quadrant}`;
    ...
    list.innerHTML = data.items
      .map(
        ...
        <article class="task-card" id="task-${task.id}">
          <div>
            <h3>${task.title}</h3>
            <div class="task-meta">${task.category} | Q${task.quadrant} | 创建: ${task.created_at} | 更新: ${task.updated_at}</div>
  ...
  ["flt-keyword", "flt-category", "flt-quadrant", "flt-sort"].forEach((id) => {
...
async function renderEditor() {
  ...
  if (state.editId) {
    const data = await api(`/api/tasks?sort=created_desc`);
  ...
  page.innerHTML = `
    ...
    <div class="editor-card">
      <form id="editor-form" class="form">
        <label>任务标题 <input id="ed-title" required value="${editing ? editing.title : ""}" /></label>
        <label>任务描述 <textarea id="ed-desc">${editing ? editing.description : ""}</textarea></label>
        <div class="editor-grid">
          <label>分类
            <select id="ed-category">${state.categories
              .map((c) => `<option ${editing && editing.category === c ? "selected" : ""}>${c}</option>`)
              .join("")}</select>
          </label>
          <label>象限
...
async function bootstrapApp() {
  ...
  state.user = me.user;
  const cats = await api("/api/meta/categories");
